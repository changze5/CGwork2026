...
@ti.func
def scene_intersect(ro, rd, exclude_sphere=True, exclude_cone=True):
    """测试光线与场景中所有物体的相交，返回最近交点"""
    min_t = 1e10
...
def render(): 
    for i, j in pixels: 
        ...
        if min_t < 1e9: 
            ...
            # --- 硬阴影计算 ---
            shadow_ray_dir = L
            shadow_ro = p + N * 1e-3  # 偏移避免自相交
            light_distance = (light_pos - p).norm()
            
            shadow_t = scene_intersect(shadow_ro, shadow_ray_dir, 
                                      exclude_sphere=not hit_sphere, 
                                      exclude_cone=hit_sphere)
            
            in_shadow = False
            if shadow_t > 0 and shadow_t < light_distance:
                in_shadow = True
...
def main(): 
    window = ti.ui.Window("Blinn-Phong Shading Demo with Hard Shadows", (res_x, res_y)) 
